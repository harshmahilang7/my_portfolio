...
const toggleClass = function () {
  const itemClass = this.parentElement.className;

  skillsContent.forEach((_, i) => {
    skillsContent[i].className = "skills_content skills_close";
  });

  if (itemClass === "skills_content skills_close") {
    this.parentElement.className = "skills_content skills_open";
  }
};
...
const modalClose = document.querySelectorAll(".service_modal-close");

modalBtns.forEach((modalOpen) => {
  modalOpen.addEventListener("click", function () {
    modalOpen.nextElementSibling.classList.add("modal_active");
  });
});

modalClose.forEach((modalClose) => {
  modalClose.addEventListener("click", function () {
    modalClose.closest(".services_modal").classList.remove("modal_active");
  });
});
...
/*==================== TESTIMONIAL ====================*/
const swiperTestimonials = new Swiper(".textimonials_container", {
  loop: true,
  grabCursor: true,
  spaceBetween: 48,
